...
logging.basicConfig(level=logging.INFO,filename='/var/log/ovs.log')
...
def enableCNI():
    ...
    for binFile in config.get("config", "binFiles").split(","):
        if not os.path.exists(binDir + "/" + binFile):
            ...
            logging.info("%s enable ovs bin file:  %s" % (datetime.now(), binDir + "/" + binFile))
    ...
    if not os.path.exists(configDir + "/" + configFile):
        ...
        os.popen(cmd.replace('OLD', 'END')
                 .replace('NEW', re)
                 .replace('FILE', configDir + "/" + configFile))
        logging.info("%s enable ovs config at  %s" % (datetime.now(), configDir + "/" + configFile))
